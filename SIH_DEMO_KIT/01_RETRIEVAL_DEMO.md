...
- **Easiest / reliable (recommended):** use the included **`assets\demo_evidence.raw`** — a ready‑made image with **12 real, openable files of 9 types** (4 JPEG photos, PNG, GIF, BMP, PDF, DOCX, SQLite, WAV, ZIP — every one opens in Windows after export), one JPEG carrying a harmless **EICAR** malware test signature in its comment field. Known‑good result: **12 recovered across 5 file families, all validated, 1 flagged as a threat.** Regenerate it any time with `node tools/demo/make-demo-image.mjs`; add `--from <folder>` to embed your own photos and documents as well.
- **Most convincing "we deleted it and got it back" (optional, now built into the app):** plug in a small USB, copy a few photos, **delete them + empty the Recycle Bin**, then image the USB **read‑only** *inside SHUNYA itself* — **no FTK Imager needed**. On the welcome screen click **Capture a device (read‑only)**, pick the USB, choose an output `.raw` file **on a different drive** (e.g. your Desktop), pick **First 2 GB** (fast, for a freshly‑prepared drive where the files sit at the start) or **Entire device**, then **Capture**. It clones the drive read‑only and shows a **SHA‑256** of the image. Then add that `.raw` in a recovery case and recover the photos you just "lost."
  - **Admin note:** capturing a physical device needs the app **running as Administrator** (same as the wipe). The included `demo_evidence.raw` needs no admin, so if you're presenting Part A without admin, use that.
  - *(FTK Imager still works too if you prefer — `File → Create Disk Image → Raw (dd)` — but the built‑in capture keeps everything in one tool.)*

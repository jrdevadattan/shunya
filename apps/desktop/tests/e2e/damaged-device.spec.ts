...
test('damaged device flow refuses to simulate unavailable ddrescue progress', async () => {
  ...
  try {
    ...
    await expect(page.getByText('Rescue Mode only', { exact: true })).toBeVisible();
    await expect(page.getByText('In Installed Mode, stop using the source and restart SHUNYA in Rescue Mode.', { exact: true })).toBeVisible();

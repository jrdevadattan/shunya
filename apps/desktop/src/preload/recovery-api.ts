export const recoveryApiMethodNames = [
  ...
  'getArtifact', 'requestPreview', 'exportArtifacts', 'generateReport', 'subscribeJobEvents',
] as const;
...
export function createRecoveryApi(transport: PreloadTransport) {
  return Object.freeze({
    ...
    subscribeJobEvents: (listener: (event: unknown) => void) => transport.subscribe('job.event', listener),
  });
}

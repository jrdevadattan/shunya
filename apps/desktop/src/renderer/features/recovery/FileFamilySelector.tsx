...
export function FileFamilySelector({ selected, onChange, disabled = false }: {
  ...
  return <fieldset className="family-selector" disabled={disabled}>
    ...
    <header className="family-selector__header">
      <div>
        <strong>File types to look for</strong>
        <p>{selected.length ? `${selected.length} of ${ALL_FILE_FAMILIES.length} families · ${formatCount(selected)} signatures` : 'Select at least one family.'}</p>
      </div>
    ...
    <div className="family-grid">
      {FILE_FAMILIES.map(({ id, label, formats, Icon, caution }) => {

...
async function loadCompleteSelection(caseId: string, onProgress: (count: number) => void): Promise<ExportSelection> {
  ...
  const requested = requestedIds.length ? new Set(requestedIds) : undefined;
...
function readRequestedIds(caseId: string): string[] { try { const value = JSON.parse(sessionStorage.getItem(`recovery:${caseId}:exportArtifactIds`) ?? '[]'); return Array.isArray(value) && value.every((item) => typeof item === 'string') ? value : []; } catch { return []; } }

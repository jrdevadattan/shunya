...
export function ExportWizard() {
  ...
  return <section className="workflow-page export-wizard"><header className="page-heading"><div><p className="eyebrow">Verified export</p><h1>Export recovered files</h1><p className="page-heading__description">The daemon proves destination topology before copying and verifies every exported hash.</p></div></header>
    {error ? <p className="form-error" role="alert">{error}</p> : null}
    <div className="export-workspace"><SurfaceCard title="Export selection" description={`${selected.length} of ${page.totalCount} indexed artifacts selected`}><form className="export-form" onSubmit={submit}><fieldset disabled={loading}><legend>Select recovered artifacts</legend>{loading ? <p role="status">Loading all recovered artifacts…</p> : null}<div className="export-selection-list">{page.items.map((artifact) => <label key={artifact.artifactId}><input type="checkbox" checked={selected.includes(artifact.artifactId)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, artifact.artifactId] : current.filter((id) => id !== artifact.artifactId))} />{artifact.displayName}</label>)}</div></fieldset><label>Export destination path<input name="destinationPath" required /></label><label>Destination physical identity<input name="destinationPhysicalId" required /></label><label className="export-form__acknowledgement"><input name="acknowledgeUnsafe" type="checkbox" /> I authorize controlled export of potentially unsafe selected files.</label><button className="button button--primary" type="submit" disabled={loading || running || selected.length === 0}>{running ? 'Exporting and verifying…' : 'Start verified export'}</button></form></SurfaceCard>
      <aside className="export-guidance"><CapabilityBanner level="warning" title="Separate destination required" explanation="Export is refused unless the daemon can prove the source and destination use separate physical topology." /></aside></div>
    {result ? <div role={completelyVerified ? 'status' : 'alert'}><SurfaceCard title={completelyVerified ? 'Export complete' : 'Verification incomplete'} className="export-completion"><p>Export ID: {result.exportId}</p><p>{completelyVerified ? `${verifiedCount} ${result.items.length === 1 ? 'file' : 'files'} exported and verified.` : `${verifiedCount} of ${submittedCount} selected files were returned and verified. Treat the export as incomplete.`}</p><ul>{result.items.map((item) => <li key={item.artifactId}>{item.outputPath} · {item.verified ? 'SHA-256 verified' : 'Verification failed'}</li>)}</ul><p>The destination was not opened automatically.</p></SurfaceCard></div> : null}
  </section>;
}

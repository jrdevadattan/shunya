...
import { File, FileArchive, FileImage, FileText, Shapes } from 'lucide-react';
...
export function ArtifactTable({ artifacts, selected, exportSelection, onSelect, onToggleExport }: {
  ...
  return <div className="artifact-table"><table aria-label="Recovered artifacts"><thead><tr><th scope="col"><span className="sr-only">Export selection</span></th><th scope="col">Name</th><th scope="col">Original path</th><th scope="col">Type</th><th scope="col">Size</th><th scope="col">Condition</th></tr></thead><tbody>{artifacts.map((artifact) => {
    ...
    return <tr className={selected === artifact.artifactId ? 'artifact-row is-selected' : 'artifact-row'} key={artifact.artifactId} aria-selected={selected === artifact.artifactId}>

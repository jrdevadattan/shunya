...
import { familyMeta, familyOf, formatLabel } from '../recovery/file-families.js';
...
export function ArtifactTable({ artifacts, selected, exportSelection, onSelect, onToggleExport }: {
  ...
  return <div className="artifact-table"><table aria-label="Recovered artifacts"><thead><tr><th scope="col"><span className="sr-only">Export selection</span></th><th scope="col">Name</th><th scope="col">Original path</th><th scope="col">Type</th><th scope="col">Size</th><th scope="col">Condition</th><th scope="col">Threat scan</th></tr></thead><tbody>{artifacts.map((artifact) => {
    ...
    const { Icon, label } = familyMeta(bucket);
    ...
    return <tr className={selected === artifact.artifactId ? 'artifact-row is-selected' : 'artifact-row'} key={artifact.artifactId} aria-selected={selected === artifact.artifactId} data-threat={threatened || undefined}>
      ...
      <td><button type="button" className="artifact-row__select" onClick={() => onSelect(artifact.artifactId)}><span className="artifact-row__icon" data-family={bucket} aria-hidden="true"><Icon /></span><span><strong>{artifact.displayName}</strong><small>{artifact.recoveryMethod === 'carving' ? 'Content-signature recovery' : 'File record recovery'}</small></span></button></td>
      <td className="artifact-row__path">{path}</td>
      <td><span className="type-badge" data-family={bucket} title={artifact.mimeType ?? label}>{formatLabel(artifact)}</span></td>
      <td className="artifact-row__size">{formatBytes(BigInt(artifact.sizeBytes))}</td>
      <td><ResultStatusBadge status={badgeStatus(artifact.recoveryState)} /></td>

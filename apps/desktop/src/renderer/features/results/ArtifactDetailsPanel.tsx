...
export function ArtifactDetailsPanel({ artifact, preview, previewError }: { artifact?: RecoveryArtifact; preview?: PreviewDescriptor; previewError?: string }) {
  ...
  return <aside className="artifact-details" aria-label={`Evidence for ${artifact.displayName}`}>
    ...
    <section className="artifact-evidence" aria-labelledby="artifact-evidence-title"><h3 id="artifact-evidence-title">Evidence supporting recovery</h3><ul>
      <li><MapPin aria-hidden="true" /><span><strong>{carved ? 'Original name and folder unavailable' : artifact.originalPath ? 'Original path from a surviving file record' : 'Original path unavailable'}</strong><small>{carved ? 'Found by its content, so the old name is unknown. Give it a name when you export.' : artifact.originalPath ?? 'The recovery service did not return path provenance.'}</small></span></li>
...
function threatDetail(threat: RecoveryArtifact['threatStatus'], name: string): string {
  if (threat === 'potential_threat') return `A threat signature matched inside "${name}". It stays quarantined and is never opened here.`;
  ...
  return 'Scanned and matched no threat rule. Evidence, not a guarantee of safety.';
}

...
export function ResultFilters({ artifacts, search, method, onSearch, onMethod, onFolder }: {
  artifacts: RecoveryArtifact[]; search: string; method?: string;
  ...
  return <aside className="results-filters">
    ...
    <nav aria-label="Recovered artifact folders and filters"><section aria-labelledby="recovered-data-title"><header><h2 id="recovered-data-title">Recovered data</h2><span>{artifacts.length} loaded</span></header><ul className="result-filter-list">
      ...
      {folders.map((folder) => <FolderBranch key={folder.path} folder={folder} onFolder={onFolder} />)}
...
function FolderBranch({ folder, onFolder }: { folder: FolderNode; onFolder(path: string): void }) {
  return <li className="result-folder-branch"><button type="button" onClick={() => onFolder(folder.path)}><Folder aria-hidden="true" /><span>{folder.label}</span><small>{folder.count}</small></button>{folder.children.length ? <ul>{folder.children.map((child) => <FolderBranch key={child.path} folder={child} onFolder={onFolder} />)}</ul> : null}</li>;
}

...
fn validate_virtual_fixture(path: &Path) -> Result<PathBuf, String> {
    ...
    {
        return Err(format!(
            "physical device targets are forbidden: {}",
            path.display()
        ));
    }
    ...
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(format!(
            "virtual fixture must be a regular non-symlink file: {}",
            path.display()
        ));
    }
    let extension = path
    ...
    if !matches!(extension.as_str(), "img" | "raw" | "dd" | "vhd" | "vhdx") {
        return Err(format!(
            "unsupported virtual fixture type: {}",
            path.display()
        ));
    }
    path.canonicalize()
...
async fn explicit_image_fixture_is_read_only_and_identity_stable() {
    let fixture =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../testdata/generated/raw-pattern.img");
    ...
    reader
        .verify_unchanged()
        .expect("source identity is stable");
    let after = sha256(&fs::read(&fixture).expect("read fixture after smoke"));
    assert_eq!(
        after, before,
        "smoke test must not mutate its source fixture"
    );
}

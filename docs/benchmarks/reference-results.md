...
| Metadata scan | filesystem corpus records elapsed time per fixture; no cross-device claim until BOSS/Windows CI baselines are retained |
...
| Large result review memory | collect Electron process working set in platform CI; release is blocked on unbounded growth, not an invented fixed marketing number |
